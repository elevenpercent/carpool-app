...
export default function RegisterPage() {
  ...
  const [avail, setAvail] = useState<Record<string, DayAvail>>(
    Object.fromEntries(
      DAYS.map((d) => [d, { can_drop: false, can_pickup: false, drop_time: "07:30", pickup_time: "15:00" }])
    )
  );
  ...
  function addKid() {
    setKids((prev) => [...prev, { name: "", grade: "K" }]);
  }
  function removeKid(i: number) {
    setKids((prev) => prev.filter((_, idx) => idx !== i));
  }
  function updateKid(i: number, k: keyof Kid, v: string) {
    setKids((prev) => prev.map((kid, idx) => (idx === i ? { ...kid, [k]: v } : kid)));
  }

  function updateAvail(day: string, k: keyof DayAvail, v: boolean | string) {
    setAvail((prev) => ({ ...prev, [day]: { ...prev[day], [k]: v } }));
  }
  ...
  async function handleSubmit() {
    ...
    const availability = DAYS.filter(
      (d) => avail[d].can_drop || avail[d].can_pickup
    ).map((day) => ({
      day,
    ...
    router.push("/schedule?registered=1");
  }
        ...
        <div className="bg-white rounded-2xl shadow-sm p-8">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input className="input" placeholder="e.g. 123 Maple St, Springfield" value={parent.address}
                ...
                <div>
                  <label className="label">Car Seats Available (for other kids)</label>
          ...
          {step === 2 && (
            <div>
                ...
                {kids.map((kid, i) => (
                  <div key={i} className="flex gap-3 items-start">
                    ...
                    {kids.length > 1 && (
                      <button
                        onClick={() => removeKid(i)}
                        className="mt-2 text-gray-300 hover:text-red-400 transition"
                      >
                        ✕
                      </button>
                    )}
              ...
              <button onClick={addKid} className="mt-3 text-sm text-indigo-600 hover:underline">
                + Add another child
              </button>
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-sm text-gray-400 mb-6">
                Which days can you drive? Check drop-off and/or pick-up.
              </p>
              <div className="space-y-4">
                {DAYS.map((day) => (
                  <div key={day} className="border border-gray-100 rounded-xl p-4">
                    <div className="font-medium text-gray-800 mb-3">
                      {DAY_LABELS[day] === "Mon" ? "Monday" :
                       DAY_LABELS[day] === "Tue" ? "Tuesday" :
                       DAY_LABELS[day] === "Wed" ? "Wednesday" :
                       DAY_LABELS[day] === "Thu" ? "Thursday" : "Friday"}
                    </div>
                    <div className="flex flex-wrap gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={avail[day].can_drop}
                        ...
                        {avail[day].can_drop && (
                          <input
                            type="time"
                            value={avail[day].drop_time}
                            onChange={(e) => updateAvail(day, "drop_time", e.target.value)}
                            className="border rounded px-2 py-1 text-xs ml-1 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                          />
                        )}
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
              ...
              {!canSubmit && (
                <p className="text-xs text-amber-500 mt-3">
                  Select at least one day you can drive (drop-off or pick-up).
                </p>
              )}
              {error && (
              ...
              <div className="flex gap-3 mt-6">
                ...
                >
                  {loading ? "Registering..." : "Register & View Schedule"}
                </button>
              </div>
            </div>
          )}
